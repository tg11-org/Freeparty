...
from apps.core.network import safe_fetch
    ...
    with observe_celery_task(self, correlation_id=correlation_id):
        try:
            ...
            if response_code >= 400:
                terminal = 400 <= response_code < 500 and response_code != 429
                ...
                err = httpx.HTTPStatusError(
                    f"HTTP {response_code}", request=response.request, response=response
                )
                mark_task_execution_failed(
                    execution=execution,
                    error=err,
                ...
                if not terminal and retries < max_retries and not getattr(self.request, "called_directly", False):
                    raise self.retry(exc=err, countdown=federation_retry_delay_seconds(retries))
                return
